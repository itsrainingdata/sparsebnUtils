...
test_that("random.psd runs as expected", {
    expect_error(random.psd(1))
    expect_error(random.psd(2), NA)
})

check_evals <- function(in_evals, check_evals, nnode){
    test_evals <- in_evals
    out_evals <- eigen(random.psd(nnode, test_evals))$values
    err <- sum(abs(out_evals - check_evals))
    err < zero_threshold()
}

test_that("random.psd gives correct results", {
